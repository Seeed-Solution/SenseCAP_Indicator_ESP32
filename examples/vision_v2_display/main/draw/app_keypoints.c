...
void process_and_draw_keypoints(cJSON* receivedJson, lv_obj_t* canvas) {
    ...
    cJSON_ArrayForEach(keypoint_json, keypoints_json) {
        ...
        if (cJSON_IsArray(points_json)) {
            ...
            lv_port_sem_take();
            draw_keypoints(canvas, current_keypoint, current_keypoint->points_count);
            lv_port_sem_give();
        }
    }
...
void draw_keypoints(lv_obj_t* canvas, const keypoints_t* keypoints, size_t count) {
    for (size_t i = 0; i < count; i++) { // 遍历所有人
        const keypoints_t* kp = &keypoints[i];
        // 定义颜色
        lv_color_t color_head = lv_color_make(255, 0, 0); // 红色，头部
        lv_color_t color_body = lv_color_make(0, 255, 0); // 绿色，身体
        lv_color_t color_legs = lv_color_make(0, 0, 255); // 蓝色，腿部

        // 绘制关键点
        switch (kp->points_count) {
        case 17: {
            // // ESP_LOGI(TAG, "17 keypoints");
            // lv_color_t color_head = lv_color_make(255, 0, 0); // For head parts
            // lv_color_t color_body = lv_color_make(0, 255, 0); // For body
            // lv_color_t color_legs = lv_color_make(0, 0, 255); // For legs
            // for (size_t point_index = 0; point_index < kp->points_count - 1; point_index++) {
            //     const keypoint_t* point = &kp->points[point_index];
            //     const keypoint_t* next_point = &kp->points[point_index + 1];
            //     lv_canvas_draw_line(canvas, point->x, point->y, next_point->x, next_point->y, color_head, 2,
            //     draw_dsc);
            // }
            // lv_canvas_draw_line(canvas, (lv_point_t*)kp->points, kp->points_count, &line_dsc);

            // for (int idx = 0; idx < (17 - 1); idx++) {
            //     int x_sta = kp->points[idx].x;
            //     int y_sta = kp->points[idx].y;

            //     int x_end = kp->points[idx + 1].x;
            //     int y_end = kp->points[idx + 1].y;
