...
static void __indicator_image_task(void *p_arg)
{
    ...
    {
        unsigned char *decoded_str = malloc(output_len + 1); // 分配足够的内存
        ...
        if (decoded_str != NULL)
        {
            ...
            if (ret == 0)
            {
                ...
                lv_img_set_src(ui_Image1, &img_png);
