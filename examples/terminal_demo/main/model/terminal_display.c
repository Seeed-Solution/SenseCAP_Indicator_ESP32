...
static struct terminal_display __g_display_model;
...
{
    esp_err_t ret = 0;
    ret = terminal_storage_write(DISPLAY_CFG_STORAGE, (void *)p_data, sizeof(struct view_data_display));
    if( ret != ESP_OK ) {
...
static void __display_cfg_restore(void)
{
    ...
    ret = terminal_storage_read(DISPLAY_CFG_STORAGE, (void *)&cfg, &len);
...
{
   __lcd_bl_on();
   terminal_display_sleep_restart();
   return 0; 
}
