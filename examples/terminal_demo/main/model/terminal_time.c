...
static struct terminal_time __g_time_model;
...
{
    esp_err_t ret = 0;
    ret = terminal_storage_write(TIME_CFG_STORAGE, (void *)p_cfg, sizeof(struct view_data_time_cfg));
    if( ret != ESP_OK ) {
...
static void __time_cfg_restore(void)
{
    ...
    ret = terminal_storage_read(TIME_CFG_STORAGE, (void *)&cfg, &len);
