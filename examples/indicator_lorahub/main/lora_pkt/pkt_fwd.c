...
#define SNTP_SERVER_IP_ADDR 162.159.200.1 /* pool.ntp.org */

#define LNS_TTN_URL eu1.cloud.thethings.network
#define LNS_TTN_IP 63.34.215.128 /* eu1.cloud.thethings.network */
#define LNS_CHIRPSTACK_URL chirpstack.az1.lorage.ovh
#define LNS_CHIRPSTACK_IP 51.137.64.106 /* chirpstack.az1.lorage.ovh */

#define DEFAULT_SNTP_SERVER SNTP_SERVER_URL
#define DEFAULT_SERVER LNS_CHIRPSTACK_URL
...
static int parse_radio_configuration( void )
{
    ...
    else
    {
        ...
        if( err == ESP_OK )
        {
            ...
            {  // TODO: check depending on modulation (FSK...)
    ...
    /* Modulation config */
    rxif_conf.modulation = MOD_LORA; /* TODO */
    ...
    while( !exit_sig )
    {
        ...
        for( i = 0; i < nb_pkt; ++i )
        {
            ...
            else if( p->modulation == MOD_FSK )
            {
                ...
                j = snprintf( ( char* ) ( buff_up + buff_index ), TX_BUFF_SIZE - buff_index, ",\"datr\":%lu",
                              p->datarate );
                if( j > 0 )
                {
                    buff_index += j;
    ...
    while( !exit_sig )
    {
        ...
        while( ( int ) difftimespec( recv_time, send_time ) < keepalive_time )
        {
            ...
            else if( strcmp( str, "FSK" ) == 0 )
            {
                ...
                if( val == NULL )
                {
                    ESP_LOGW( TAG_DOWN, "WARNING: [down] no mandatory \"txpk.fdev\" object in JSON, TX aborted\n" );
                    json_value_free( root_val );
                    continue;
                }
                txpkt.f_dev =
                    ( uint8_t )( json_value_get_number( val ) / 1000.0 ); /* JSON value in Hz, txpkt.f_dev in kHz */
                ...
                {
                    i = ( int ) json_value_get_number( val );
                    if( i >= MIN_FSK_PREAMBLE )
                    {
                        txpkt.preamble = ( uint16_t ) i;
                    }
                    else
                    {
                        txpkt.preamble = ( uint16_t ) MIN_FSK_PREAMBLE;
                    }
                }
                else
                {
                    txpkt.preamble = ( uint16_t ) STD_FSK_PREAMBLE;
                }
            }
            else
            {
...
int launch_pkt_fwd(temperature_sensor_handle_t temperature_sensor)
{
    ...
    i = pthread_create(&thrid_pktfwd, NULL, task_wrapper, NULL);
    if (i != 0)
    {
