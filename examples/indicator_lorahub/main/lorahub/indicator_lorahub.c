...
static void __view_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    switch (id) {
        ...
        case VIEW_EVENT_WIFI_CONNECT_RET:{
            ESP_LOGI(TAG, "VIEW_EVENT_WIFI_CONNECT_RET");
            struct view_data_wifi_connet_ret_msg *view_wifi_msg = (struct view_data_wifi_connet_ret_msg *)event_data;
            ESP_LOGI(TAG, "WiFi connection %s", view_wifi_msg->ret ? "failed" : "successful");
            if(!view_wifi_msg->ret)
            {
...
int indicator_lorahub_init(void)
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register_with(view_event_handle,
                                                            VIEW_EVENT_BASE, VIEW_EVENT_WIFI_CONNECT_RET,
                                                            __view_event_handler, NULL, NULL));
