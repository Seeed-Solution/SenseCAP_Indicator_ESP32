...
{
    esp_err_t ret = 0;
    ret = indicator_storage_write(LORAWAN_BASIC_CFG_STORAGE, (void *)p_data, sizeof(struct view_data_lorawan_basic_cfg));
    if( ret != ESP_OK ) {
...
{
    esp_err_t ret = 0;
    ret = indicator_storage_write(LORAWAN_AUTO_JOIN_FLAG_STORAGE, (void *)&flag, sizeof(flag));
    if( ret != ESP_OK ) {
...
{
    esp_err_t ret = 0;
    ret = indicator_storage_write(LORAWAN_LIGHT_FLAG_STORAGE, (void *)&st, sizeof(st));
    if( ret != ESP_OK ) {
...
static void __lorawan_cfg_restore(void)
{
    ...
    ret = indicator_storage_read( LORAWAN_BASIC_CFG_STORAGE, (void *)&cfg, &len);
    ...
    ret = indicator_storage_read(LORAWAN_NETWORK_INFO_STORAGE, (void *)&info, &len);
    ...
    len = 1;
    ret = indicator_storage_read(LORAWAN_AUTO_JOIN_FLAG_STORAGE, (void *)&auto_join, &len);
    ...
    bool light_st = false;
    len = 1;
    ret = indicator_storage_read(LORAWAN_LIGHT_FLAG_STORAGE, (void *)&light_st, &len);
    ...
    if( light_st ) {
        lorawan_data[LORAWAN_DATA_INDEX_LIGHT].vaule   = 1.0;
    } else {
        lorawan_data[LORAWAN_DATA_INDEX_LIGHT].vaule   = 0.0;
    }
...
static void PrepareTxFrame( void )
{
    ...
    for( i = 0; i < LORAWAN_DATA_INDEX_MAX; i++) {
        if( lorawan_data[i].is_valid ) {
            ...
            uint32_t value = 0;
            memcpy(&value, &lorawan_data[i].vaule, 4);
        ...
        case VIEW_EVENT_SENSOR_DATA: {
            ...
            switch (p_data->sensor_type)
            {
                case SENSOR_DATA_CO2: {
                    ...
                    lorawan_data[LORAWAN_DATA_INDEX_CO2].vaule   = p_data->vaule;
                    break;
                }
                case SENSOR_DATA_TVOC: {
                    lorawan_data[LORAWAN_DATA_INDEX_TVOC].is_valid= true;
                    lorawan_data[LORAWAN_DATA_INDEX_TVOC].vaule   = p_data->vaule;
                    break;
                }           
                case SENSOR_DATA_TEMP: {
                    lorawan_data[LORAWAN_DATA_INDEX_TEMP].is_valid= true;
                    lorawan_data[LORAWAN_DATA_INDEX_TEMP].vaule   = p_data->vaule;
                    break;
                }
                case SENSOR_DATA_HUMIDITY: {
                    lorawan_data[LORAWAN_DATA_INDEX_HUMIDITY].is_valid= true;
                    lorawan_data[LORAWAN_DATA_INDEX_HUMIDITY].vaule   = p_data->vaule;
                    break;
                }
                default:
                    break;
